...
function App() {
  ...
  const [TodaysCardioWorkouts, setTodaysCardioWorkouts] = useState<TodaysCardioWorkouts[]>([]);
...
const PostWorkoutData = async () => {
  try {
    ...
    if (newWorkout.setDetails.length > 1) {
      const remainingPromises = newWorkout.setDetails.slice(1).map((set, index) => {
  ...
  const PostCardioWorkouttData = async () => {
    try {
      const totalSeconds: number = (Number(newCardio.hour) * 3600) +
  ...
  const GoToStatus = () => {
    navigate({ to: "/savedroutines/$username", params: { username: username } });
  };
  ...
  const GoTohistory = () => {
    navigate({ to: "/previousworkouts/$username", params: { username: username } });
  };
  ...
  return (
    <ProtectedRoute>
      <div className="bg-gradient-to-r from-[#6D51A5] to-[#E4A7C5] home-container flex flex-col items-center min-h-svh bg-gray-100 p-4 overflow-y-auto">
                ...
                <div className="flex flex-col space-y-2 p-2 border rounded">
                  ...
                  <div className="relative">
                    <Input
                      ...
                      onChange={(e) => {
                        // Update workout form state
                        setNewWorkout({ ...newWorkout, activity: e.target.value });
                          ...
                          {activitySuggestions.map((suggestion, index) => (
                            <li
                              ...
                              onClick={() => {
                                ...
                                const matchingWorkout = AllWorkouts.find(w => w.activity === suggestion);
                                // if (matchingWorkout) {
                                //   setNewWorkout({
                  ...
                  {newWorkout.setDetails.length > 0 && (
                    <div className="space-y-4 mt-4">
                      ...
                      {newWorkout.setDetails.map((set, index) => (
                        <div key={index} className="flex space-x-2 p-2 border rounded">
                          ...
                          <div className="flex-1">
                            <Label htmlFor={`weight-${index}`} className="text-xs">Weight (Kgs)</Label>
                            <Input
                              id={`weight-${index}`}
                              value={set.weight}
                              onChange={(e) => {
                    ...
                    {TodaysCardioWorkouts && TodaysCardioWorkouts.length > 0 ? (
                      TodaysCardioWorkouts.map((workout) => (
                        <TableRow key={workout.id}>
                          ...
                          <TableCell className="text-center">
                            Time: {formatTime(workout.time)} | Distance: {workout.distance} {" "}kms
                          </TableCell>
                        </TableRow>
                      ))
                    ) : (
        ...
        <Button className="m-6" variant={"destructive"}
